...
class RoleApplyView(discord.ui.LayoutView):
    ...
    kmb_button = discord.ui.Button(label="УКМБ", emoji="🔰", style=discord.ButtonStyle.secondary, custom_id="role_apply_kmb")
...
async def _apply_role_discord(interaction: Interaction[ClientT], request: RoleRequest, user_discord: discord.Member):
    ...
    if request.role_type in (RoleType.ARMY, RoleType.KMB):
        ...
        role_ids = [config.RoleId.MILITARY.value, config.RANK_ROLES[config.RANKS[0]], extra_role, config.RoleId.DIV.value, config.RoleId.POS.value]
